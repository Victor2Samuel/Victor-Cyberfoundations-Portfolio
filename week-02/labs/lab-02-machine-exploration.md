...
```
58 GB of 477 GB used
```

**Free storage:** 419 GB

```
477 - 58 = 419 GB free storage left
```
...
**My Screenshot:** https://github.com/Victor2Samuel/Victor-Cyberfoundations-Portfolio/blob/main/assets/screenshots/week-02/Task%20manager.PNG
...
```
The task manager I watched shows me how my software and OS are using the RAM and Storage in real time. 
RAM (kitchen island) rep the Memory tab (task manager ): shows how space an app is using 
Storage (pantry) rep the Disk tab (task manager): shows the speed of how an app reads files  (mb/s)
OS: all these tasks are running on the OS.
The Hardware (physical components) are the CPU (11%), RAM (68%) and Storage (0%)
OS version: is coordinating the hardware pieces of the computer (Windows 11 pro, version 23H2
```
...
```
The Software cannot communicate with the hardware without the OS, the software sends instructions to the OS, the OS receives the instructions and translates it into a task and the Hardware does the job. So the Software depends on the OS and Hardware to gets its job done.
```
...
```
The constantly changing numbers on the CPU tab means the protection on the OS and Software is working properly and continuously to detect any security vulnerabilities/ threats which is what we want. If a process I don’t recognize suddenly spike CPU usage it indicates a malicious software program is detected in the hardware.
```
...
```
The 3 layer diagram in Lab 01 shows similar components for example Hardware layer represents CPU, RAM, Disk. The Software & OS layers represents the Processes and Apps.
The Task manager shows me in real time the relationship between the 3 components while the static diagram shows me a fixed structured view of how the components are within the computer. It shows how the computer is built. While the task manager shows me what the computer is doing in realtime.
```
